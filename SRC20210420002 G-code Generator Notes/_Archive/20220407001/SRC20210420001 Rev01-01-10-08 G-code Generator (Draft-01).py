...
def line(end_x, end_y, name, feed = None, ramp_z = None, end_z = None):
    ...
    # cutter_x, cutter_y, cutter_z, text = line(end_x, end_y, name, feed, ramp_z, end_z)
...
def arc(end_x, end_y, name, rad, cw, less_180, feed = None, ramp_z = None, end_z = None):
    ...
    # Optional feed rate.
    # returns cutter position
    # cutter_x, cutter_y, cutter_z, text = arc(end_x, end_y, name, rad, cw, less_180, feed, ramp_z, end_z)
...
def toolpath_data_frame(name, excel_file, sheet, debug = False):
    # ---Description---
    ...
    def extract_row(counter, debug=False):
        ...
        return (x, y, z, rad, arc_seg, cw, less_180, tri, ramp_z, mode, operation, offset, step, wos, feed, dia)  # return values
    ...
    start_x, start_y, start_z, discard, discard, discard, discard, discard, discard, mode, operation, offset, step, wos, feed, dia = extract_row(counter)   # extract row 0 values.
    counter = counter + 1                                                                                                   # increment counter
    end_x, end_y, end_z, rad, arc_seg, cw, less_180, tri, ramp_z, mode, operation, offset, step, wos, feed, dia = extract_row(counter)                    # extract row 1 values.
    ...
    while counter <= last_row:      # recursive loop
        ...
        if tri == False:
            if arc_seg == False:
                cutter_x, cutter_y, cutter_z, text_temp = line(end_x_adjusted, end_y_adjusted, name, feed, ramp_z, end_z)  # print G-code for adjusted line segment.
            else:
                cutter_x, cutter_y, cutter_z, text_temp = arc(end_x_adjusted, end_y_adjusted, name, rad_adjusted, cw, less_180, feed, ramp_z, end_z)  # print G-code for adjusted arc segment.
        elif tri == True:
            if arc_seg == False:
...
#            text = f'''\n(!!script aborted!!)\n(invalid trichoidal selection)\n(row = {counter})\n(column = tri)\n'''  # write header for section.
        ...
        end_x, end_y, end_z, rad, arc_seg, cw, less_180, tri, ramp_z, mode, operation, offset, step, wos, feed, dia = extract_row(counter)
...
#clear_z = 6.000  # safe z
#initial_x = 0  # initial x
#initial_y = 0  # initial y
#start_z = 0.000  # start z
#terminal_x = 0  # terminal x
#terminal_y = 0  # terminal y
#cut_f = 181.1  # cutting feed rate
#z_f = 27.2  # plunge feed rate
#finish_f = 108.7 # finish feed rate
#rpm = 2900  # spindle speed
#cutter_dia = 1.0  # diameter of cutter before adjustment
#tol = 0.0    # adjustment for cutter tolerance
#dia = cutter_dia + tol  # adjust cutter tolerance
#loc = 3.0    # length of cutter
#flute = 4   # number of flutes
#surface_speed = 15  #Surface Speed (m/min)
#chipload = 0.007 # Chipload (mm/tooth)
#cutter_material = "carbide"    # material of cutter. e.g. HSS, carbide, cobalt
#coating = "AT"    # coating of cutter.  e.g. None, AT, TiN
...
#z_origin = "top surface of vise"    # z origin e.g. top surface of part, top surface of vise
...
start_x, start_y, end_x, end_y, cutter_x, cutter_y, text_tri = toolpath_data_frame(name, excel_file, sheet, debug)
...
start_x_tri, start_y_tri, end_x, end_y, cutter_x, cutter_y, text_tri = toolpath_data_frame(name, excel_file, sheet, debug)

#offset = 0.1
#feed = finish_f
excel_file = 'vector-01.xlsx'
sheet = 'Sheet12'
start_x_fin, start_y_fin, end_x, end_y, cutter_x, cutter_y, text_fin = toolpath_data_frame(name, excel_file, sheet, debug)
