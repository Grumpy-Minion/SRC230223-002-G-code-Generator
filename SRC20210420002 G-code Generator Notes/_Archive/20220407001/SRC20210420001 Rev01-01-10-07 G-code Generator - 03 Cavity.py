...
#        typea = 'N/A'
    ...
    end_x, end_y, end_z, rad, arc_seg, cw, less_180, tri, ramp_z, mode, operation = extract_row(counter)                    # extract row 1 values.
...
#            text = f'''\n(!!script aborted!!)\n(invalid trichoidal selection)\n(row = {counter})\n(column = tri)\n'''  # write header for section.
        ...
        end_x, end_y, end_z, rad, arc_seg, cw, less_180, tri, ramp_z, mode, operation = extract_row(counter)

# ---------File Name Variables------------

doc_number = datetime.now().strftime("%Y%m%d-%H%M%S")  # get date time stamp (YYYYMMDD-HHMMSS) for file name.
prefix = 'GCE'
file_name = 'PZT Bracket-03 Cavity'
rev = '01'
...
start_x_cavity, start_y_cavity, end_x, end_y, cutter_x, cutter_y, text_cavity = toolpath_data_frame(dia, offset, step, wos, feed, name, excel_file, sheet, debug)
...
start_x_pocket, start_y_pocket, end_x, end_y, cutter_x, cutter_y, text_pocket = toolpath_data_frame(dia, offset, step, wos, feed, name, excel_file, sheet, debug)
...
text_cavity_start = \
    f'''
G0 Z{"%.4f" % clear_z}				(Rapid to safe height)
G0 X{"%.4f" % start_x_cavity} Y{"%.4f" % start_y_cavity}                 (Rapid to start point)
G0 Z{"%.4f" % doc}				(Rapid to cutting height)
F{cut_f}     (set cutting feed)
'''
...
text_pocket_start = \
    f'''
G0 Z{"%.4f" % clear_z}				(Rapid to safe height)
G0 X{"%.4f" % start_x_pocket} Y{"%.4f" % start_y_pocket}                 (Rapid to start point)
G0 Z{"%.4f" % doc}				(Rapid to cutting height)
F{cut_f}     (set cutting feed)
'''
...
while i <= 3:
    ...
    text = text_cavity_start + text_cavity
    write_to_file(name, text)

    text = text_pocket_start + text_pocket
    write_to_file(name, text)
