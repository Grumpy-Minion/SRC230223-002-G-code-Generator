...
def toolpath_data_frame(name, excel_file, sheet, start_safe_z, return_safe_z, debug = False):
    # ---Description---
    ...
    def extract_row(counter, debug=False):
        ...
        # x, y, z, rad, arc_seg, cw, less_180, tro, mode, operation, offset, step, wos, feed, dia, doc, safe_z, z_f = extract_row(counter)
...
#        tro = format_data_frame_variable(df, 'tro', counter, debug)  # import trochodial flag.
#        ramp_z = format_data_frame_variable(df, 'ramp_z', counter, debug)  # import ramp_z.
        ...
        offset = format_data_frame_variable(df, 'offset', counter, debug)  # import offset.
        step = format_data_frame_variable(df, 'step', counter, debug)  # import step.
        wos = format_data_frame_variable(df, 'wos', counter, debug)  # import wos.
        feed = format_data_frame_variable(df, 'feed', counter, debug)  # import feed.
        dia = format_data_frame_variable(df, 'dia', counter, debug)  # import dia.
        doc = format_data_frame_variable(df, 'doc', counter, debug)  # import doc.
        safe_z = format_data_frame_variable(df, 'safe_z', counter, debug)  # import safe_z.
        z_f = format_data_frame_variable(df, 'z_f', counter, debug)  # import z_f.

        if operation=='tro-arc' or operation=='arc':
            arc_seg = True
        elif operation=='tro-slot' or operation=='line':
            arc_seg = False
        else:
            arc_seg = None

        if operation=='tro-slot' or operation=='tro-arc':
            tro = True
        elif operation=='line' or operation=='arc':
            tro = False
        else:
            tro = None

        return (x, y, z, rad, arc_seg, cw, less_180, tro, mode, operation, offset, step, wos, feed, dia, doc, safe_z, z_f)  # return values
    ...
    start_x, start_y, start_z, rad, arc_seg, cw, less_180, tro, mode, operation, offset, step, wos, feed, dia, doc, safe_z, z_f = extract_row(counter)   # extract row 0 values.
    counter = counter + 1                                                                                                   # increment counter
    end_x, end_y, end_z, rad, arc_seg, cw, less_180, tro, mode, operation, offset, step, wos, feed, dia, doc, safe_z, z_f = extract_row(counter)                    # extract row 1 values.
    ...
    while counter <= last_row:      # recursive loop
        ...
        if counter == 1:
            ...
            G1 Z{"%.4f" % doc}	F{"%.4f" % z_f}			(Rapid to cutting height)
...
#            text = f'''\n(!!script aborted!!)\n(invalid trochoidal selection)\n(row = {counter})\n(column = tro)\n'''  # write header for section.
        ...
        end_x, end_y, end_z, rad, arc_seg, cw, less_180, tro, mode, operation, offset, step, wos, feed, dia, doc, safe_z, z_f = extract_row(counter)
...
def parameters_data_frame(excel_file, sheet):
    ...
    # start_block, end_block, name, clear_z, start_z, cut_f, finish_f, z_f, dia = parameters_data_frame(excel_file, sheet)
    ...
    cutter_dia = format_data_frame_variable(df, 'value', 'cutter diameter')  # diameter of cutter before adjustment
    tol = format_data_frame_variable(df, 'value', 'tolerance')  # adjustment for cutter tolerance
    dia = cutter_dia + tol  # adjusted cutter tolerance
    ...
    return (start_block, end_block, name, clear_z, start_z, cut_f, finish_f, z_f, dia)
...
excel_file = 'LOG20220414001 Rev01 G-code Parameters.xlsx'       # identify name of excel file to import data from.
sheet = 'parameters'                # identify name of excel sheet to import data from.
start_block, end_block, name, clear_z, start_z, cut_f, finish_f, z_f, dia = parameters_data_frame(excel_file, sheet)        # generate G-code parameters.
...
sheet = 'main'
df_main = pd.read_excel(excel_file, sheet_name=sheet)  # import excel file, main sheet, into dataframe.
...
while counter<=last_row:
    ...
    if operation == 'line' or operation == 'trochoidal':
        ...
        start_safe_z = format_data_frame_variable(df_main, 'start_safe_z', counter)
        return_safe_z = format_data_frame_variable(df_main, 'return_safe_z', counter)
        first_x_adjusted, first_y_adjusted, end_x, end_y, cutter_x, cutter_y, text = toolpath_data_frame(name, excel_file, sheet, start_safe_z, return_safe_z, debug = False)
        write_to_file(name, text)
    ...
    elif operation == 'safe_z':
        text = f'G0 Z{clear_z}          (Safe Z)\n'
        write_to_file(name, text)
