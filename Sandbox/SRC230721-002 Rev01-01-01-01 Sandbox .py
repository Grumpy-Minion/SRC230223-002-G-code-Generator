...
#    if ((mode == 1 and cw == True) or (mode == 2 and cw == False))and dia/2+offset >= rad:
...
def temp_text_df_debug(df_profile) :
    ...
    df_temp = df_temp.to_markdown(index=False, tablefmt='pipe',floatfmt=".4f",colalign=['center'] * len(df_temp.columns))  # format df into table
...
excel_file = 'LOG230724-003 Parameters Sandbox.xlsx'       # !!!! identify name of excel file to import data from. !!!!
sheet = 'profile-00'                # identify name of excel sheet to import data from.
doc_number = datetime.now().strftime("%Y%m%d-%H%M%S")  # get date time stamp (YYYYMMDD-HHMMSS) for file name.
name_debug = 'Sandbox Debug ' + str(doc_number)     # name of debug text file
...
df_profile = pd.read_excel(excel_file, sheet_name=sheet, na_filter=False)  # import excel file into dataframe.
...
df_line = pd.read_excel(excel_file, sheet_name='line-00', na_filter=False)  # import excel file line-01 tab into dataframe.
df_line_static = df_line.loc[:,'static_variable':'static_value']        # creates new data frame for static variables only.
df_line_static.set_index('static_variable', inplace=True)  # replace index default column with 'static_variable' column
print(str(df_line_static))
print('\n')

df_line = df_line.loc[:, :'comments']  # create dataframe up to 'comments' columns
temp = df_line.to_markdown(index=False, tablefmt='pipe', colalign=['center'] * len(df_line.columns))  # tabulate dataframe
print(temp)
#print(str(df_line))
print('\n')
...
def extract_row(counter, df, debug=False):
    ...
    # Extract and format variables of a single row from the data frame to the explicit variable type.
    # returns formatted row of variables.
    # last_row_flag, x, y, z, arc_seg, rad, cw, less_180 = extract_row(counter, df)
...
while line_counter <= end :
    ...
    last_row_flag, x, y, z, arc_seg, rad, cw, less_180 = extract_row(line_counter, df_line)
    ...
    df_profile.loc[profile_counter, 'offset_start'] = df_line_static.loc['offset', 'static_value']  # write static_value to df_profile dataframe. at point of creation, offset is constant.
    df_profile.loc[profile_counter, 'offset_end'] = df_line_static.loc['offset', 'static_value']  # write static_value to df_profile dataframe. at point of creation, offset is constant.
...
#                rel_m = math.tan(rel_angle)  # calculate gradient of relative segment for y=mx+c in radians.
        ...
        # -------------------------------
            ...
            elif round(length,4) > round((prior_segment_r + later_segment_r),4):        # arc circles are external non-intersecting.
                inversion_type = 'arc-arc external non-intersecting'
            elif round(length,4) < round(abs(prior_segment_r - later_segment_r),4):     # arc circles are internal non-intersecting.
