...
def toolpath_data_frame(dia, offset, step, wos, feed, name, excel_file, sheet, debug = False):
    ...
    def extract_row(counter, debug = False):
        ...
        type = format_data_frame_variable(df, 'type', counter, debug)           # import type.

        return (x, y, z, rad, arc_seg, cw, less_180, tri, ramp_z, mode, type)         # return values
    ...
    start_x, start_y, start_z, discard, discard, discard, discard, discard, discard, mode, discard = extract_row(counter)   # extract row 0 values.
    counter = counter + 1                                                                                                   # increment counter
    end_x, end_y, start_z, rad, arc_seg, cw, less_180, tri, ramp_z, mode, discard = extract_row(counter)                    # extract row 1 values.
...
#            text = f'''\n(!!script aborted!!)\n(invalid trichoidal selection)\n(row = {counter})\n(column = tri)\n'''  # write header for section.
        ...
        end_x, end_y, start_z, rad, arc_seg, cw, less_180, tri, ramp_z, mode, discard = extract_row(counter)
