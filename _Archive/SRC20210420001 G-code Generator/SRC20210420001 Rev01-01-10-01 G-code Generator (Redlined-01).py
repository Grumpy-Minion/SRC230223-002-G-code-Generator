...
#    if mode == 1 or mode == 2:    # update adjustment offset position of slot center.
...
def line(dia, offset, start_x, start_y, end_x, end_y, name, feed = None, ramp_z = None, mode = None):
    ...
    # rev: 01-01-10-01
    # Initial release.
    # software test run on xx/Sep/2021
    # --------------------
...
def spiral_surface(origin_x, origin_y, start_dia, end_dia, doc, dia, step, z_f, cut_f, safe_z, name, debug = False):
    ...
    # spiral_surface(origin_x, origin_y, start_dia, end_dia, doc, dia, step, z_f, cut_f, safe_z, name, debug)
...
#    length = round(length, 4)  # Round to 4 decimal places
...
#        x= round(x,4)       # Round to 4 decimal places
#        y= round(y,4)       # Round to 4 decimal places
#        length = round(length, 4)      # Round to 4 decimal places
        text = \
    f'''
    ...
    '''
        ...
        if last == True:
            i = origin_x-x
#            i = round(i, 4)  # Round to 4 decimal places
            j = origin_y-y
#            j = round(j, 4)  # Round to 4 decimal places
            text = \
    f'''
    G3 X{"%.4f" % x} Y{"%.4f" % y} I{"%.4f" % i} J{"%.4f" % j}
    (spiral surface end)
    '''
            write_g_code(name, text)
            break
...
def spiral_boss(origin_x, origin_y, start_dia, end_dia, doc, dia, step, z_f, cut_f, safe_z, name, debug = False):
    ...
    # return to safe z after surfacing.
    # spiral_boss(origin_x, origin_y, start_dia, end_dia, doc, dia, step, z_f, cut_f, safe_z, name, debug)
...
#            j = round(j, 4)  # Round to 4 decimal places
            text = \
    f'''
    G2 X{"%.4f" % x} Y{"%.4f" % y} I{"%.4f" % i} J{"%.4f" % j}
    (spiral surface end)
    '''
            write_g_code(name, text)
            break
...
def test_cut(dia, name, length_x = 22, length_y = 22, surface_block = True, hole = True):
    ...
    def test_block(dia, doc):
        ...
        start_x = -9.00  # x start of slot along neutral axis
        start_y = 10.00  # y start of slot along neutral axis
        ...
        G0 X{start_x} Y{10 + wos / 2 + offset}     (rapid to start point!!!)
        ...
        next_x, next_y, cutter_x, cutter_y = tri_slot(start_x, start_y, 9.000, 10.000, step, wos, dia, offset, name,
                                                      start_x, start_y, True, False, mode, False)
        next_x, next_y, cutter_x, cutter_y = tri_arc(next_x, next_y, 10.000, 9.000, step, wos, dia, offset, 1.000, True,
                                                     True, name, cutter_x, cutter_y, False, False, mode, debug=False)
        next_x, next_y, cutter_x, cutter_y = tri_slot(next_x, next_y, 10.000, 1.000, step, wos, dia, offset, name,
                                                      cutter_x, cutter_y, False, False, mode, False)
        next_x, next_y, cutter_x, cutter_y = tri_arc(next_x, next_y, 9.000, 0.000, step, wos, dia, offset, 1.000, True,
                                                     True, name, cutter_x, cutter_y, False, False, mode, debug=False)
        next_x, next_y, cutter_x, cutter_y = tri_slot(next_x, next_y, 1.000, 0.000, step, wos, dia, offset, name,
                                                      cutter_x, cutter_y, False, False, mode, False)
        next_x, next_y, cutter_x, cutter_y = tri_arc(next_x, next_y, 0.000, 1.000, step, wos, dia, offset, 1.000, True,
                                                     True, name, cutter_x, cutter_y, False, False, mode, debug=False)
        next_x, next_y, cutter_x, cutter_y = tri_slot(next_x, next_y, 0.000, 9.000, step, wos, dia, offset, name,
                                                      cutter_x, cutter_y, False, False, mode, False)
        next_x, next_y, cutter_x, cutter_y = tri_arc(next_x, next_y, 1.000, 10.000, step, wos, dia, offset, 1.000, True,
        ...
        end_x, end_y, end_x_adjusted, end_y_adjusted = line(dia, offset, -(0 + dia + 1), 10, 1, 10, name, feed, ramp_z,
                                                            mode)
        ...
        while i <= 3:  # run 3 finish passes
            ...
            end_x, end_y, end_x_adjusted, end_y_adjusted = line(dia, offset, end_x, end_y, 9, 10, name, feed, ramp_z,
                                                                mode)
            end_x, end_y, end_x_adjusted, end_y_adjusted = arc(dia, offset, end_x, end_y, 10, 9, name, rad, cw,
                                                               less_180, feed, ramp_z, mode)
            end_x, end_y, end_x_adjusted, end_y_adjusted = line(dia, offset, end_x, end_y, 10, 1, name, feed, ramp_z,
                                                                mode)
            end_x, end_y, end_x_adjusted, end_y_adjusted = arc(dia, offset, end_x, end_y, 9, 0, name, rad, cw, less_180,
                                                               feed, ramp_z, mode)
            end_x, end_y, end_x_adjusted, end_y_adjusted = line(dia, offset, end_x, end_y, 1, 0, name, feed, ramp_z,
                                                                mode)
            end_x, end_y, end_x_adjusted, end_y_adjusted = arc(dia, offset, end_x, end_y, 0, 1, name, rad, cw, less_180,
                                                               feed, ramp_z, mode)
            end_x, end_y, end_x_adjusted, end_y_adjusted = line(dia, offset, end_x, end_y, 0, 9, name, feed, ramp_z,
                                                                mode)
            end_x, end_y, end_x_adjusted, end_y_adjusted = arc(dia, offset, end_x, end_y, 1, 10, name, rad, cw,
                                                               less_180, feed, ramp_z, mode)
...
"tri_slot"
"Expand for details"
# cutter_x, cutter_y, start_x, start_y = tri_slot(start_x, start_y, end_x, end_y, step, wos, dia, name, cutter_x, cutter_y, first_slot=True, last_slot=True, debug = False)
...
"tri_arc"
"Expand for details"
# cutter_x, cutter_y, start_x, start_y = tri_arc(start_x, start_y, end_x, end_y, step, wos, dia, rad_slot, cw, less_180, name, cutter_x, cutter_y, first_slot=True, last_slot=True, debug = False)
...
"surface"
"Expand for details"
# surface(origin_x, origin_y, length_x, length_y, doc, dia, step, z_f, cut_f, safe_z, name, debug = False)
...
"spiral_drill"
"Expand for details"
# spiral_drill(origin_x, origin_y, dia_hole, depth, step_depth, dia, z_f, cut_f, safe_z, name, debug = False)
...
"spiral_surface"
"Expand for details"
# spiral_surface(origin_x, origin_y, start_dia, end_dia, doc, dia, step, z_f, cut_f, safe_z, name, debug = False)
...
"corner_slice"
"Expand for details"
# corner_slice(start_x, start_y, end_x, end_y, start_rad, end_rad, doc, dia, step, z_f, cut_f, safe_z,name, mode = 0, debug = False)
...
"spiral_boss"
"Expand for details"
# spiral_boss(origin_x, origin_y, start_dia, end_dia, doc, dia, step, z_f, cut_f, safe_z, name, debug = False)
...
dia = dia + tol  # adjust cutter tolerance
loc = 12    # length of cutter
flute = 4   # number of flutes
cutter_material = "..."    # material of cutter. e.g. HSS, carbide, cobalt
coating = "..."    # coating of cutter.  e.g. None, AT, TiN
x_origin = "..."      # x origin e.g. center of part, left edge
y_origin = "..."    # y origin. e.g. center of part, bottom edge
z_origin = "..."    # z origin e.g. top surface of part, top surface of vise
part_material = "..."   # material of part. e.g. delrin, ABS, SS304, CoCr
...
(TOOL/MILL, dia: {"%.3f" % dia}, LOC:{"%.3f" % loc}, {flute} Flute, {cutter_material}, {coating})
(--X origin: {x_origin}--)
(--Y origin: {y_origin}--)
(--Z origin: {z_origin}--)
...
def linear_test():
    ...
    text = \
    f'''
    G0 X{"%.4f" % start_x} Y{"%.4f" % start_y}
    G0 X{"%.4f" % end_x} Y{"%.4f" % end_y}
    '''
    write_g_code(name,text)
    ...
    offset=1
    mode=1
    start_x, start_y, end_x, end_y = linear_offset_adjustment(dia, offset, start_x, start_y, end_x, end_y, mode)
...
def test_arc():
    ...
    start_x_1, start_y_1, end_x_1, end_y_1, rad_1 = arc_offset_adjustment(dia, offset, start_x, start_y, end_x, end_y, rad, cw, less_180, mode)

    text = \
        f'''
        G0 X{"%.4f" % start_x} Y{"%.4f" % start_y}
        G2 X{"%.4f" % end_x} Y{"%.4f" % end_y} R{"%.4f" % -rad}
        G0 X{"%.4f" % start_x_1} Y{"%.4f" % start_y_1}
        G2 X{"%.4f" % end_x_1} Y{"%.4f" % end_y_1} R{"%.4f" % rad_1}
        '''
    write_g_code(name, text)

#test_arc()

def line_offset_test():
    dia = 3
    offset = 1
    start_x = 0
    start_y = 0
    ...
    text = f'''G0 X{"%.4f" % start_x_adjusted} Y{"%.4f" % start_y_adjusted}\n'''
    write_g_code(name,text)
    line(dia, offset, start_x, start_y, end_x, end_y, name, feed, ramp_z, mode)
...
def arc_offset_test():
    ...
    text = f'''G0 X{"%.4f" % start_x_adjusted} Y{"%.4f" % start_y_adjusted}\n'''
    write_g_code(name,text)
    arc(dia, offset, start_x, start_y, end_x, end_y, name, rad, cw, less_180, feed, ramp_z, mode)
